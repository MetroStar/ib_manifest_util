...
update_repo(
    ...
    local_env_path=repo_dir.joinpath(
        "scripts", "local_channel_env.yaml"
    ),  # defaults to the one in `repo_dir`
    output_hardening_path=output_dir.joinpath(
        "output_hardening_manifest.yaml"
    ),  # defaults to overwrite the one in `repo_dir`
    output_dockerfile_path=output_dir.joinpath(
        "output_dockerfile"
    ),  # defaults to overwrite the one in `repo_dir`
